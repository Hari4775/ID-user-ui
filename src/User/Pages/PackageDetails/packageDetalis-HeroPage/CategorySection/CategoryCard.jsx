...
const CategoryCard = ({ category }) => {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `url(${
          category.categoryImage || "default-placeholder.png"
        })`,
      }}
    >
      <div className="lg:mt-4 md:px-5 px-1 md:pb-5 pb-1">
        ...
        <div className="lg:mt-2  flex items-center  rounded-lg px-2 bg-slate-300">
          ...
          <div>
            <span className="line-through text-gray-500 lg:text-sm text-[10px] font-bold">
              ₹{category?.categoryRegularPrice}
            </span>
            <span className="lg:text-sm text-[10px] font-bold text-green-500 mx-2">
              ₹{category?.categoryDiscountedPrice}
            </span>
          </div>
        </div>
        <div className=" w-full h-full mt-5">
          <button
            className="bottom-2 left-2 view-button text-white px-3 py-1  lg:text-sm text-xs"
